...
import com.ardy.consumerfavapp.adapter.FavAdapter
import com.ardy.consumerfavapp.databinding.ActivityDetailgithubBinding
import com.ardy.consumerfavapp.db.FavContract
import com.ardy.consumerfavapp.db.FavContract.FavColumns.Companion.CONTENT_URI
import com.ardy.consumerfavapp.db.FavHelperQuery
import com.ardy.consumerfavapp.entity.FavDat
import com.ardy.consumerfavapp.helper.MappingHelper
...
class Detailgithub : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if ( intent?.getParcelableExtra(ARG_section_username) as? Github != null ) {
            ...
            GlobalScope.launch(Dispatchers.Main) {
                ...
                val deferredNotes = async(Dispatchers.IO) {
                    val cursorr = contentResolver.query(CONTENT_URI, null, null, null, null)
                    MappingHelper.mapCursorToArrayList(cursorr)
                }
                val notes = deferredNotes.await()
                adapter.listNotes = notes
                for(i in 0 until adapter.listNotes.size){
                    if (user.username == adapter.listNotes[i].username.toString()) {
                        statusFav = true
                        setstatfav(true)
        ...
        if ( intent?.getParcelableExtra(EXTRA_USER) as? FavDat != null) {
            ...
            favhelper = FavHelperQuery.getInstance(applicationContext)
            favhelper.open()
            uriWithId = Uri.parse(CONTENT_URI.toString() + "/" + userr?.id.toString())
                ...
                .setPositiveButton("Ya") { _, _ ->
                    if (isDialogClose) {
                        ...
                        if ( intent?.getParcelableExtra(EXTRA_USER) as? FavDat != null) {
                            ...
                            Toast.makeText(this, "Satu item berhasil dihapus", Toast.LENGTH_SHORT)
                                .show()
